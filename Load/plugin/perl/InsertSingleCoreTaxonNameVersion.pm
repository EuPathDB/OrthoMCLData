package OrthoMCLData::Load::Plugin::InsertSingleCoreTaxonNameVersion;
...
[
 stringArg({name           => 'abbrev',
            descr          => '4-letter abbreviation for the core species. this must be unique',
            reqd           => 1,
            constraintFunc => undef,
            isList         => 0, }),

 stringArg({ descr => 'four-letter orthomcl clade to which the core species belongs',
	     name  => 'orthomclClade',
	     isList    => 0,
	     reqd  => 1,
	     constraintFunc => undef,
 ...
 stringArg({ descr => 'The NCBI taxon id of the core species',
	     name  => 'ncbiTaxonId',
	     isList    => 0,
	     reqd  => 1,
	     constraintFunc => undef,
...
my $purpose = <<PURPOSE;
Insert core species info into ApiDB::OrthomclTaxon table. The taxon_id is obtained from the NCBI taxon id.
PURPOSE

my $purposeBrief = <<PURPOSE_BRIEF;
Insert core species info into ApiDB::OrthomclTaxon table.
PURPOSE_BRIEF

my $notes = <<NOTES;
Need to enter info into the dataset file regarding the core species, into OrthoMCL.xml file.
NOTES
